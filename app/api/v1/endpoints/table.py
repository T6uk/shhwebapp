...
@router.get("/deposit-data")
async def get_deposit_data(db: AsyncSession = Depends(get_db)):
    """Get deposit data for the deposit view"""
    try:
        # For now, return empty structure - will be populated by CSV import
        columns = [
            "Toimiku nr lõplik", "Kande kpv", "Arvelduskonto nr", "Panga tunnus",
            "Nimi pangas", "Nimi baasis", "Nõude sisu", "Toimiku jääk",
            "Staatus baasis", "Toimikute arv tööbaasis", "Toimikute jääkide summa tööbaasis",
            "Vahe", "Elatus-miinimumid", "Laekumiste arv", "Laekumised kokku",
            "Tagastamised", "S/V", "Summa", "Viitenumber", "Arhiveerimistunnus",
            "Makse selgitus", "Valuuta", "Isiku- või registrikood",
            "Isiku- või registrikood baasist", "Laekusmise tunnus",
            "Laekusmise kood deposiidis", "Kliendi konto", "EM märkus", "Toimiku märkused"
        ]
        ...
        logger.exception(f"Error getting deposit data: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error getting deposit data: {str(e)}")


@router.post("/import-deposit-csv")
async def import_deposit_csv(request: Request, db: AsyncSession = Depends(get_db)):
    """Import CSV file for deposit data - FIXED VERSION"""
    try:
        # Get the request body
        try:
            body = await request.json()
            logger.info(f"Received import request body: {body}")
        except Exception as e:
        ...
        logger.info(f"Attempting to import CSV file: {file_path}")
        ...
        # Check if file is from the konto vv folder
        is_konto_vv = "konto vv" in file_path.lower()
        logger.info(f"Is konto vv file: {is_konto_vv}")
        ...
        try:
            ...
            try:
                ...
                logger.info(f"CSV header (first 5 columns): {header[:5] if header else 'No header'}")
            ...
            for row_num, row in enumerate(csv_reader, start=2):
                ...
                # Skip rows that are essentially empty (all fields empty or very short)
                ...
                try:
                    if is_konto_vv:
                        ...
                        # Map columns according to the specification with safe indexing
                        row_data["Kliendi konto"] = row[0] if len(row) > 0 else ""
                        # row[1] is ignored
                        row_data["Kande kpv"] = row[2] if len(row) > 2 else ""
                        row_data["Arvelduskonto nr"] = row[3] if len(row) > 3 else ""
                        row_data["Nimi pangas"] = row[4] if len(row) > 4 else ""
                        row_data["Panga tunnus"] = row[5] if len(row) > 5 else ""
                        # row[6] is ignored
                        row_data["S/V"] = row[7] if len(row) > 7 else ""
                        # rows 8, 9 are ignored
                        row_data["Viitenumber"] = row[10] if len(row) > 10 else ""
                        row_data["Arhiveerimistunnus"] = row[11] if len(row) > 11 else ""

                        # Extract only {nr}/{nr}/{nr} part from 13th column
                        makse_selgitus = row[12] if len(row) > 12 else ""
                        if makse_selgitus:
                            match = re.search(r'\d+/\d+/\d+', makse_selgitus)
                            row_data["Makse selgitus"] = match.group(0) if match else ""
                        else:
                            row_data["Makse selgitus"] = ""

                        # Initialize other columns as empty
                        for col in ["Toimiku nr lõplik", "Nimi baasis", "Nõude sisu", "Toimiku jääk",
                                    "Staatus baasis", "Toimikute arv tööbaasis", "Toimikute jääkide summa tööbaasis",
                                    "Vahe", "Elatus-miinimumid", "Laekumiste arv", "Laekumised kokku",
                                    "Tagastamised", "Summa", "Valuuta", "Isiku- või registrikood",
                                    "Isiku- või registrikood baasist", "Laekusmise tunnus",
                                    "Laekusmise kood deposiidis", "EM märkus", "Toimiku märkused"]:
                            if col not in row_data:
                                row_data[col] = ""
                    else:
                        logger.debug(f"Processing other folder row {row_num}")
                        # For other folders, populate available data or initialize as empty
                        for i, col in enumerate(["Toimiku nr lõplik", "Kande kpv", "Arvelduskonto nr", "Panga tunnus",
                                                 "Nimi pangas", "Nimi baasis", "Nõude sisu", "Toimiku jääk",
                                                 "Staatus baasis", "Toimikute arv tööbaasis",
                                                 "Toimikute jääkide summa tööbaasis",
                                                 "Vahe", "Elatus-miinimumid", "Laekumiste arv", "Laekumised kokku",
                                                 "Tagastamised", "S/V", "Summa", "Viitenumber", "Arhiveerimistunnus",
                                                 "Makse selgitus", "Valuuta", "Isiku- või registrikood",
                                                 "Isiku- või registrikood baasist", "Laekusmise tunnus",
                                                 "Laekusmise kood deposiidis", "Kliendi konto", "EM märkus",
                                                 "Toimiku märkused"]):
                            row_data[col] = row[i] if len(row) > i else ""
            ...
            return {
                ...
                "data": data,
                "file_type": "konto_vv" if is_konto_vv else "other",
        ...
        logger.exception(f"Unexpected error importing CSV: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Unexpected error importing CSV: {str(e)}")


@router.get("/browse-deposit-folder")
async def browse_deposit_folder(path: str = ""):
    """Browse the deposit folder structure"""
    try:
        ...
        logger.info(f"Browsing folder: {full_path}")
